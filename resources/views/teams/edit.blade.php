<x-app-layout title="Modifier l'équipe">
    <x-slot name="header">
        <x-page-header :title="'Modifier ' . $team->name" />
    </x-slot>
    ...
    <x-card class="max-w-xl">
        <form method="POST" action="{{ route('teams.update', $team) }}" class="space-y-5">
            ...
            <div>
                ...
                <x-textarea-input id="description" name="description" rows="2">{{ old('description', $team->description) }}</x-textarea-input>
            ...
            <div>
                ...
                <div class="mt-1 max-h-56 space-y-1 overflow-y-auto rounded-lg border border-sand-200 p-3 dark:border-ink-700">
                    @php $memberIds = $team->users->pluck('id'); @endphp
                    @forelse ($members as $member)
                        <label class="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-sand-50 dark:hover:bg-ink-800">
                            <input type="checkbox" name="members[]" value="{{ $member->id }}" @checked($memberIds->contains($member->id)) class="rounded border-sand-300 text-ink-900 focus:ring-ink-500 dark:border-ink-600 dark:bg-ink-800">
